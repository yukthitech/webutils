...
export var $modelDefService = {
	...
	/**
	 * Divides displayable fields into rows of {@code columnCount} and populates field component details.
	 * Used by search forms and simple model dialogs.
	 */
	"divideModelRows": function(modelDef, modelFieldRows, columnCount) {
		...
		for(var fieldDef of modelDef.fields)
		{
			...
			if(!lastRow || lastRow.fields.length >= columnCount)
			{
				lastRow = {"fields": [], "index": modelFieldRows.length};
				modelFieldRows.push(lastRow);
			}
